...
public class RecordSearchJob extends AbstractJob {
	...
	public RecordSearchJob(TableViewEditorFor31 editor, String inputCondition, String filterOrderBy, int offset, int limit) {
	...
	protected IStatus run(IProgressMonitor monitor) {
		...
		try {
			...
			String[] conditions = SQLFormatter.splitOrderCause(inputCondition);
			String condition = conditions[0];		
			String orderBy = null;		
			String orderBy1 = conditions[1]; // for Input Where Statement
			String orderBy2 = filterOrderBy; // for FilterDialog
			
			if(orderBy1 != null && !"".equals(orderBy1) && orderBy2 != null && !"".equals(orderBy2)){
				orderBy = orderBy1;
			}else if(orderBy1 != null && !"".equals(orderBy1) ){
				orderBy = orderBy1;
			}else if(orderBy2 != null && !"".equals(orderBy2) ){
				orderBy = orderBy2;
			}
			...
			if (orderBy != null) {
				...
				if (condition == null) {
					elements = TableManager.invoke(table.getDbConfig(), table, null, offset, limit);
				} else {
					elements = TableManager.invoke(table.getDbConfig(), table, condition, offset, limit);
				}
			...
			if (limit == 0) {
				showResults(new ShowResultAction(inputCondition, elements, time.getTotalTime(), false));
				...
				if (factory.isSupportPager()) {
					showResults(new ShowResultAction(inputCondition, elements, time.getTotalTime(), true));
				} else {
					
					//showResults(new ShowResultAction(condition, elements, time.getTotalTime(), false));
					showResults(new ShowResultAction(inputCondition, elements, time.getTotalTime(), true));	// [change] doCalculate is true
				}
			}
			...
			showResults(new ShowResultAction(inputCondition, elements, time.getTotalTime(), e.getMessage(), true));
