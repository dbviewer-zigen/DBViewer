		...
		try {
			...
			if ("SYNONYM".equals(lable)) { //$NON-NLS-1$
				...
				case DBType.DB_TYPE_ORACLE:
					...
					Synonym _synonym = new Synonym(tableInfo.getName(), tableInfo.getComment(), tableInfo.getTimestamp());
				...
				View nTable = new View(tableInfo.getName(), tableInfo.getComment(), tableInfo.getTimestamp());
				if (tableNode instanceof Bookmark) {
				...
				// Update
				Table nTable = new Table(tableInfo.getName(), tableInfo.getComment(), tableInfo.getTimestamp());
				if (tableNode instanceof Bookmark) {
					((Bookmark) tableNode).update(nTable);
				} else {
			...
			if(tableNode.getUpdTime() != null){
				DbPlugin.getDefault().getITableSerializeManager().delete(tableNode);
			}
