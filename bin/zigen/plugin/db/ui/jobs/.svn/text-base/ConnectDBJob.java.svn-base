	...
	private void addTables(TreeNode parent, String label, TableInfo[] tables) {
		...
		for (int i = 0; i < tables.length; i++) {
			...
			if ("VIEW".equals(label)) { //$NON-NLS-1$
				table = new View(tableinfo.getName(), tableinfo.getComment(), tableinfo.getTimestamp());

			} else {
				table = new Table(tableinfo.getName(), tableinfo.getComment(), tableinfo.getTimestamp());
			}
