...
public class TableTypeSearchJob extends AbstractJob {
	...
	protected IStatus run(IProgressMonitor monitor) {
		try {
			...
			setExtentionElement(conifg, schema);
	...
	private void setExtentionElement(IDBConfig config, Schema schema) {
		final String ExtentionPointName = "typePoint";
		final String ExtentionName = "element";
			...
			for (int i = 0; i < extensions.length; i++) {
				...
				for (int j = 0; j < cfgElems.length; j++) {
					...
					if(ExtentionName.equals(cfgElem.getName())){
							...
							String type = cfgElem.getAttribute("type");
							String wDriverName = config.getDriverName().toLowerCase();
							if(wDriverName.indexOf(name) >= 0){
								schema.addSourceType(type);
								Folder folder = new Folder(type);
