...
public class TableViewEditorFor31 extends MultiPageEditorPart implements ITableViewEditor, IPropertyChangeListener, ISelectionListener, IStatusChangeListener, IPageChangeListener {
	...
	protected String preWhereString;
	...
	private void createMainPage() {
		...
		changeColorJob.setPriority(ChangeColorJob.SHORT);
	...
	protected void conditionEventHandler(KeyEvent e) {
		if (e.character == SWT.CR) {
			...
			preWhereString = whereString;

			pager.setPageNo(1);
			this.offset = pager.getOffset();
			limit = DbPlugin.getDefault().getPreferenceStore().getInt(PreferencePage.P_MAX_VIEW_RECORD);
			updateTableViewer(whereString, this.offset, limit);
		}
	}

	public void pageChanged(int status, int _offset, int limit) {
		this.limit = limit;
		this.offset = _offset;
		//this.whereString = conditionComb.getText();
		//updateTableViewer(whereString, this.offset, limit);
		updateTableViewer(preWhereString, this.offset, limit);
	...
	private void createToolBar(Composite parent) {
		...
		searchItem.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				limit = DbPlugin.getDefault().getPreferenceStore().getInt(PreferencePage.P_MAX_VIEW_RECORD);
				updateTableViewer(preWhereString, pager.getOffset(), limit);
			}
		});
...
//		}
		if (preWhereString != null && !"".equals(preWhereString)) { //$NON-NLS-1$
			conditionComb.setText(preWhereString);
		}
	...
	protected void updateTableViewer(String inputCondition, int _offset, int limit) {
		...
		RecordSearchJob job = new RecordSearchJob(this, inputCondition,  getOrderByString(), offset, limit);
		...
		synchronized (lock) {
			...
			if (viewer == null || isSearch) {
				...
				if (elements != null) {
					...
					job2.setPriority(RecordCountForTableJob.SHORT);
					job2.setUser(false);
					job2.schedule();
	...
	void setDDLString() {
		try {
			...
			ddlViewer.getDocument().set(factory.createDDL(true));
		} catch (Exception e) {
			DbPlugin.log(e);
		}
	}
			...
			try {
				...
				synchronized (loadingColumnInfolock) {
					if (!table.isExpanded()) {
						...
						ts.stop();
						System.out.println("カラム情報取得処理時間　" + ts.getTotalTime());
					}
				}
