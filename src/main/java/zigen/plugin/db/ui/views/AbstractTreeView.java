...
public abstract class AbstractTreeView extends ViewPart implements IStatusChangeListener {
	...
	protected Button tableSearchBtn;

	protected Button columnSearchBtn;
	...
	protected void createTreeArea(Composite parent) {
		...
		viewer.setLabelProvider(new DecoratingTreeLabelProvider(new TreeLabelProvider()));
	...
	protected void addColumnFilter(Composite tool) {
		columnFilterLabel = new Label(tool, SWT.NONE);
		...
		if (!isAutoSearchMode) {
			...
			columnFilterComb.addMouseListener(new MouseListener(){
	...
	protected void addTableFilter(Composite tool) {
		tableFilterLabel = new Label(tool, SWT.NONE);
		GridData gridData = new GridData(GridData.FILL);
		...
		if (!isAutoSearchMode) {
			...
			tableFilterComb.addMouseListener(new MouseListener(){
			    public void mouseDoubleClick(MouseEvent mouseevent){
			    	tableFilter(tableFilterComb.getText());
			    }
