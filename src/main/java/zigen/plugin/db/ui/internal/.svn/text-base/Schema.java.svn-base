...
public class Schema extends TreeNode {
	...
	public void addSourceType(String sourceType) {
		if (this.sourceTypes != null) {
			List list = new ArrayList(this.sourceTypes.length + 1);
			for (int i = 0; i < this.sourceTypes.length; i++) {
				list.add(this.sourceTypes[i]);
			}
			list.add(sourceType);
			this.sourceTypes = (String[]) list.toArray(new String[0]);
		} else {
			this.sourceTypes = new String[]{sourceType};
		}
	}
