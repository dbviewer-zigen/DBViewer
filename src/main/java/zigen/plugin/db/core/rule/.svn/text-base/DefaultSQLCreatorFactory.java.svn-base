...
public class DefaultSQLCreatorFactory extends AbstractSQLCreatorFactory implements ISQLCreatorFactory {
	...
	protected DefaultSQLCreatorFactory() {
	}
	...
	public void setTable(ITable table) {
		if (table != null) {
			...
			this.nuidxs = convertTableIDXColumn(table.getTableNonUIDXColumns());
	...
	public String createDDL(boolean withComment) {
	...
	public String createCountAll(String _condition) {
		StringBuffer sb = new StringBuffer();
	...
	public String getTableComment() {
		...
		if(sb.length() > 0){
			sb.insert(0, DbPluginConstant.LINE_SEP);
		}
		return sb.toString();
	}
	...
	public String getIndex() {
		...
		Constraint pk = null;
		if (pks != null && pks.length > 0) {
			pk = new Constraint(pks);
		}

		StringBuffer sb = new StringBuffer();

		for (Iterator iter = nuidxs.iterator(); iter.hasNext();) {
			...
			if (index != null && pk != null && index.getName().equals(pk.getName())) {
				...
				sb.append(index.getDDL(isVisibleSchemaName, encloseChar));
				setDemiliter(sb);
			}
		}
