...
public abstract class AbstractMappingFactory implements IMappingFactory {
	...
	private static IMappingFactory getStandardFactory(String driverName, boolean isConvertUnicode){
		IMappingFactory factory = null;
	...
	private static IMappingFactory getExtentionFactory(String driverName, boolean isConvertUnicode){
		final String ExtentionPointName = "mappingFactoryPoint";
					...
					if(ExtentionName.equals(cfgElem.getName())){
						try {
							...
							if(wDriverName.indexOf(name) >= 0){
								if(obj instanceof IMappingFactory){
									factory= (IMappingFactory)obj;
									factory.setConvertUnicode(isConvertUnicode);
									return factory;

								}else{
									DbPlugin.log("The mistake is found in plugin.xml.(implements IMappingFactory)");
								}
							}
