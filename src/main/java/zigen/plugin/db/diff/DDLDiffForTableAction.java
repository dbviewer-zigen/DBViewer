...
public class DDLDiffForTableAction extends Action implements Runnable {
	...
	public void run() {
		...
		try {
			...
			if (index == 2) {
				String msg = "DDLを比較します";
				String opt = "コメントを無視する";
				...
				if (dialog.getReturnCode() == YES) {
					...
					showDDLDiff(!dialog.getToggleState());
	...
	private void showDDLDiff(boolean withComment) throws Exception {

		IDDLDiff diff = new DDLDiff(new DDL(left, withComment), new DDL(right, withComment));
