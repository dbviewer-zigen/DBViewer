...
public class DbPlugin extends AbstractUIPlugin {
	...
	private ITableSerializeManager iTableSerializeManager;
	...
	public void start(BundleContext context) throws Exception {
		...
		iTableSerializeManager = new ITableSerializeManager(getStateLocation());
		imageCacher = ImageCacher.getInstance();
	}


	private static SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd", Locale.JAPAN);
...
//		}
	...
	public ITableSerializeManager getITableSerializeManager() {
		return iTableSerializeManager;
	}
	...
	public static SqlFormatRule getSqlFormatRult() {
		...
		String demiliter = st.getString(SQLEditorPreferencePage.P_SQL_DEMILITER);
		if("/".equals(demiliter)){
			rule.setOutSqlSeparator(SqlFormatRule.SQL_SEPARATOR_SLASH);
		}else if(";".equals(demiliter)){
			rule.setOutSqlSeparator(SqlFormatRule.SQL_SEPARATOR_SEMICOLON);
		}else{
			rule.setOutSqlSeparator(SqlFormatRule.SQL_SEPARATOR_NONE);
		}
		return rule;
	}
	...
	public static final String IMG_CODE_FILE = "file.gif";//$NON-NLS-1$
	...
	protected void initializeImageRegistry(ImageRegistry registry) {
		...
		registerImage(registry, IMG_CODE_FILE);
